...
docker exec -t himpact-prod-postgres pg_dump -U himpact himpact_prod | gzip > "${BACKUP_FILE}"
...
find "${BACKUP_DIR}" -type f -name "himpact_prod_*.sql.gz" -mtime +${RETENTION_DAYS} -delete
